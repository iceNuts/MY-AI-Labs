...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for i in xrange(len(livingGhosts) - 1):
        ghostState = livingGhosts[i + 1]
        if ghostState:
            cnt += 1
            ghostDistribution = livingGhostPositionDistributions[cnt]
            maxProb = -1
            maxPos = -1
            for pos, prob in ghostDistribution.items():
                if (prob > maxProb or maxProb == -1) and prob != 0: 
                    maxProb = prob
                    maxPos = pos
            if maxPos != -1: distances[maxPos] = self.distancer.getDistance(pacmanPosition, maxPos)  
    minDistance = -1
    ...
    for action in legal:
        ...
        if miniDistance == -1 or miniDistance > self.distancer.getDistance(successorPosition, minPos):
            miniAction = action
            miniDistance = self.distancer.getDistance(successorPosition, minPos)
    return miniAction
