...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #equation
    #P(x, o1->ot) = p(x, o1 -> ot-1) * p(ot | x, o1->ot-1)
    ...
    if noisyDistance == None:
        ...
        allPossible[ghostPosition] = emissionModel[trueDistance]*self.beliefs[ghostPosition]
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    temporary = util.Counter()
    for ghostPosition, oldProb in self.beliefs.items():
        if not oldProb > 0: continue
        allPossible = util.Counter()
        ...
        for newPos, newProb in newPosDist.items():
            allPossible[newPos] = self.beliefs[ghostPosition]*newProb
            temporary[newPos] += allPossible[newPos] 
            #SigmaA P(Xt+1, A) = P(Xt+1)
    self.beliefs = temporary
    self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for i in xrange(self.numParticles):
        ...
        self.particlesWeight.append(1.0)
  ...
  def observe(self, observation, gameState):
    ...
    zero = True
    temporaryParticles = []
    temporaryParticlesWeight = []
    cnt = 0
    for pos in self.particles:
        weight = self.particlesWeight[cnt]
        cnt += 1
        distance = util.manhattanDistance(pos, pacmanPosition) 
        if weight * emissionModel[distance] != 0:
            temporaryParticles.append(pos)  
            temporaryParticlesWeight.append(weight * emissionModel[distance])
            zero = False
    ...
    self.particlesWeight = temporaryParticlesWeight
  ...
  def getBeliefDistribution(self):
    #Be careful with dictionary & index when there are same key inside
    """
    ...
    for pos in self.particles:
        ...
        cnt += 1
        if pos in tPos:
            tWeight[tPos.index(pos)] += weight
        else:
            tPos.append(pos)
            tWeight.append(weight)    
    for i in xrange(len(tPos)):    
        self.beliefs[tPos[i]] = tWeight[i]
    self.beliefs.normalize()
    return self.beliefs
...
class JointParticleFilter:
  ...
  def observeState(self, gameState):
    ...
    for particle in self.particles:
        ...
        product = 1.0
        for i in particleWeights:
            product *= particleWeights[i]
        if product != 0: zero = False
        particleCounter[particle] += product
    
    if(zero): self.initializeParticles(); return
    
    newParticles = []
    for i in xrange(len(self.particles)):
        rsample = util.sample(particleCounter)
        newParticles.append(rsample)
    self.particles = newParticles
...
def setGhostPositions(gameState, ghostPositions):
  ...
  """
  particle filter:
  #1 get uniform sample
  #2 move state with transitional model
  #3 cal current state your weight comparing with a relative model
  #4 pick(or directly normalize beliefs, it's usually used when sample set is small) new samples according to current weight
  #5 return each particle's value to draw on screen
  """
